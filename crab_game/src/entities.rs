...
pub enum FallingItemType {
    YellowStar,  // 5 points, 45% probability
    PinkStar,    // 10 points, 20% probability
    PurpleStar,  // 20 points, 10% probability
...
impl Crab {
    ...
    // Reset the crab (when game restarts)
...
impl FallingItem {
    pub fn new(
        ...
        let (item_type, img) = if rand < 0.45 {
            // 45% chance for yellow star
            (FallingItemType::YellowStar, yellow_star_img)
        } else if rand < 0.65 {
            // 20% chance for pink star
            (FallingItemType::PinkStar, pink_star_img)
        } else if rand < 0.75 {
            // 10% chance for purple star
            (FallingItemType::PurpleStar, purple_star_img)
        } else {
            // 25% chance for rock
    ...
    // Collision detection
    pub fn collides_with(&self, crab: &Crab) -> bool {
        let crab_hitbox_reduction = 30.0; // Adjust collision detection sensitivity
        
        !(self.x + self.width < crab.x + crab_hitbox_reduction || 
          self.x > crab.x + crab.width - crab_hitbox_reduction || 
          self.y + self.height < crab.y + crab_hitbox_reduction || 
          self.y > crab.y + crab.height - crab_hitbox_reduction)
    }
    
    // Get point value based on item type
    pub fn get_point_value(&self) -> i32 {
        match self.item_type {
            FallingItemType::YellowStar => 5,
            FallingItemType::PinkStar => 10,
            FallingItemType::PurpleStar => 20,
    ...
    pub fn is_star(&self) -> bool {
        match self.item_type {
            FallingItemType::YellowStar | FallingItemType::PinkStar | FallingItemType::PurpleStar => true,
