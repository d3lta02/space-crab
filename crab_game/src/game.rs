...
use crate::entities::{Crab, FallingItem, FallingItemType};
...
impl Game {
    // Add extra images to crab
    pub fn set_alternate_images(&mut self, shield_img: HtmlImageElement, double_img: HtmlImageElement) {
        // Store alternate crab images for later use
        self.crab_shield_img = shield_img;
        self.crab_double_img = double_img;
    }
    ...
    pub fn new(
        ...
        let crab = Crab::new(width, height, crab_img);
    ...
    pub fn handle_key_press(&mut self, event: KeyboardEvent) {
        if matches!(self.state, GameState::Playing) {
            let key = event.key();
            match key.as_str() {
                "ArrowLeft" | "a" | "A" => self.crab.move_left(),
                "ArrowRight" | "d" | "D" => self.crab.move_right(),
...
self.spawn_interval = (1.2 - (difficulty_level * 0.1)).max(0.4); // Minimum 0.4 seconds (increased from 0.3)
        ...
        let previous_powerup_level = (self.score - 1).max(0) / 100;
        ...
        for (i, item) in self.falling_items.iter_mut().enumerate() {
            ...
            if item.collides_with(&self.crab) {
                match item.item_type {
                    FallingItemType::YellowStar | FallingItemType::PinkStar | FallingItemType::PurpleStar => {
                        // Star collected, add points
                        ...
                        if self.sound_enabled {
                            let _ = self.star_sound.play().unwrap();
                        }
    ...
    fn spawn_falling_items(&mut self) {
        ...
        for _ in 0..num_items {
            ...
            let mut item = FallingItem::new(
                ...
                self.yellow_star_img.clone(),
                self.pink_star_img.clone(),
                self.purple_star_img.clone(),
                self.rock_img.clone(),
            );
    ...
    fn draw(&self) {
        ...
        // Determine which crab image to use
        let crab_img_to_use = if self.shield_active_until.is_some() {
            &self.crab_shield_img
        } else if self.double_points_active_until.is_some() {
            &self.crab_double_img
        } else {
            &self.crab.img
        };
        ...
        if let Some(shield_end_time) = self.shield_active_until {
            ...
            self.ctx.set_fill_style(&JsValue::from_str("rgba(100, 100, 255, 0.7)"));
            ...
            // Draw a ring around the crab if shield is active
            self.ctx.set_stroke_style(&JsValue::from_str("rgba(100, 100, 255, 0.7)"));
            self.ctx.set_line_width(3.0);
            self.ctx.begin_path();
            self.ctx.arc(
                self.crab.x + self.crab.width / 2.0,
                self.crab.y + self.crab.height / 2.0,
                self.crab.width / 2.0 + 10.0,
                0.0,
        ...
        if let Some(double_points_end_time) = self.double_points_active_until {
            ...
            self.ctx.set_fill_style(&JsValue::from_str("rgba(255, 100, 100, 0.7)"));
            self.ctx.fill_rect(10.0, y_offset, 30.0, 30.0);
            self.ctx.set_font("16px Arial");
            self.ctx.set_fill_style(&JsValue::from_str("white"));
        ...
        if let Some(slowdown_end_time) = self.slowdown_active_until {
            ...
            self.ctx.set_fill_style(&JsValue::from_str("rgba(100, 255, 100, 0.7)"));
            self.ctx.fill_rect(10.0, y_offset, 30.0, 30.0);
            self.ctx.set_font("16px Arial");
            self.ctx.set_fill_style(&JsValue::from_str("white"));
